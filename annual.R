...
plot <- ggplot(
  ...
  #              outlier.size = 0,
  #              fill = "#e2e9f8")+
  geom_quasirandom(color = "#6E92E0", shape = 21, stroke = 0.075, size = 1) +
...
mine <- old_csv |>
  ...
  mutate(
    weekday = fct_relevel(weekday, "M", "Tu", "W", "Th", "F", "Sa", "Su")
  ) |>
...
plot <- ggplot(weekday_medians) +
  geom_col(
    aes(x = weekday, y = weekday_median),
    fill = "#6E92E0",
    color = "white"
  ) +
  theme_minimal() +
...
plot <- ggplot(mine, aes(x = rank)) +
  stat_count(fill = "#6E92E0", color = "white") +
  facet_wrap(~name) +
  xlab("Rank") +
  ylab("Count") +
  theme_minimal() +
